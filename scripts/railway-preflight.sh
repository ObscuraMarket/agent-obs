...
railway variables -s desk --kv 2>/dev/null | grep -E "^(OBS_TRADING|OBS_WALLET_ADDRESS|OBS_WALLET_DIR|OBS_WALLET_JSON|OBS_PAPER|OBS_VENUE|OBS_BASE|OBS_MAX_SWAP_USD|OBS_PROBE_USD|OBS_DAILY_LOSS_USD|OBS_DAILY_LOSS_PCT|ROBINHOOD_RPC_URL|OBS_MEMORY_REPO_URL)=" | sed -E 's/^(OBS_WALLET_JSON|OBS_MEMORY_REPO_URL)=.*/\1=<set>/; s/^(ROBINHOOD_RPC_URL)=(.{28}).*/\1=\2.../' | sort

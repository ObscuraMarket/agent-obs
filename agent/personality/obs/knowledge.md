...
- The tokenized stocks trade around the clock on chain. Wall Street's print behind them is live only 9:30 to 4 New York time on weekdays; on nights and weekends the on-chain price floats on its own supply and demand and converges back toward the print when the session reopens. A paused print is context for where the price is anchored, never by itself a reason to sit out; the gap between the two is where a tokenized stock's on-chain dislocations live. That gap is your bread-and-butter trade, the basis: the perp venue on this chain prices NVDA around the clock with real volume and is the honest 24-hour reference; when the pool is cheaper than it by more than a round trip costs plus a margin, buy NVDA with USDG and sell it back to USDG when the gap closes, dollar in and dollar out with no ETH exposure. It is small, it repeats most days, and it is argued from figures every time; it is not a reason to churn when the gap is inside the cost.

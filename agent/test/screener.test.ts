...
const pool = (over: Partial<ScreenerPair> = {}): ScreenerPair => ({
  ...
  priceUsd: 0.027, vol24: 27_438_423, vol6: 3_511_531, vol1: 2_023_076, txns24: 71_065, txns1: 3056, buys1: 1437, sells1: 1619, liqUsd: 480_264, chg1: 66.9, chg24: 3398, fdvUsd: 22_449_025, pairCreatedAt: now - 24.2 * 3600e3,
  ...over,
});
...
test("a survivor becomes a grade B candidate with its record as the reason, under the same age floor as the feed", () => {
  const t: ScreenerToken = { token: ZZZ, symbol: "ZZZ", source: "pons-v2", launchAt: now - 25 * 3600e3, creatorTaxBps: 0, pool: pool(), key: { currency0: "0x0000000000000000000000000000000000000000", currency1: ZZZ, fee: 0, tickSpacing: 200, hooks: HOOK }, readAt: now - 60e3 };
  ...
  assert.match(c.record?.line ?? "", /record: 25 h old, \$27\.4M in 24h, \$3\.5M in 6h, \$2\.0M last hour, 71,065 swaps in 24h, liquidity \$480k/);

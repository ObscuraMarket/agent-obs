...
test("a wallet lock counts as held only while its pid lives and it is under the stale window, ten minutes unless set", () => {
  ...
  assert.equal(lockIsHeld({ pid: 146, at: now - 10 * M, what: "withdraw" }, now, yes), false, "ten minutes old is stale");
  assert.equal(lockIsHeld({ pid: 146, at: now - 10 * M + 1, what: "withdraw" }, now, yes), true, "a moment under ten minutes is held");
  ...
  assert.equal(lockStaleMs({ OBS_WALLET_LOCK_MIN: "0" } as NodeJS.ProcessEnv), 10 * M, "zero would make every lock stale at once: the default stands");
  assert.equal(lockStaleMs({ OBS_WALLET_LOCK_MIN: "soon" } as NodeJS.ProcessEnv), 10 * M);
...
test("a lock whose process is gone, or which is past the window, is stale and taken over; a live one is not", () => {
  ...
  try {
    ...
    dead!();
    writeFileSync(join(locks, `${A}.json`), JSON.stringify({ pid: 999_999, at: now - 10 * M, what: "withdraw" }));
    const old = acquire(A, "entry Y", { dir: locks, now, alive: yes });
    assert.ok(old, "ten minutes old with its pid alive: stale by age, taken over");
    old!();
    writeFileSync(join(locks, `${A}.json`), JSON.stringify({ pid: 999_999, at: now - 11 * M, what: "withdraw" }));
    assert.equal(acquire(A, "entry Y", { dir: locks, now, alive: yes, staleMs: 15 * M }), null, "the operator's longer window keeps it held");
  } finally {
    rmSync(dir, { recursive: true, force: true });
  }
});
...
test("a release after a takeover leaves the new holder's lock alone, and a file mid-write is held while a corrupt old one goes", () => {
  ...
  try {
    const locks = join(dir, "locks");
    const first = acquire(A, "withdraw", { dir: locks, now: now - 12 * M, pid: 4242 });
    assert.ok(first);
    const taker = acquire(A, "exit PENGUIN", { dir: locks, now, alive: yes });
    assert.ok(taker, "twelve minutes old: stale, taken over");
    first!();
    ...
    const late = acquire(A, "withdraw", { dir: locks, now: real + 11 * M });

...
export function personaFor(address: string, s: UserSettings = getSettings(address), door: Door = doorNow()): string {
  ...
  return [
    ...
    "",
    "What you are not: a trading agent. You do not trade, place orders, hold or move anything for anyone. This wallet does have a trading agent, and it is not you: it is a follower of Agent OBS that the person controls from the console. /start turns it on (with a size, /start 150), and from that moment every entry and every exit the house desk makes is mirrored on their own book at their size, at the desk's fill price. It runs on paper for now: a real book of the desk's real trades, no money moving yet, and the same commands run it live once the live path has been tested with real size. /stop turns it off (it still sells what it holds when the desk does), /size changes what it puts into each entry, /agent shows its book. It never makes a trade of its own. If they ask you to trade, buy, sell, place an order or turn trading on, point them at /start and say exactly what it does; if they ask why it is paper, say the live version is being tested with real size first.",

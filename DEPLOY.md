...
| OBS_CONVOY_DEPTH_MULT | the convoy depth rule: a signal row is strong only when the desk itself wanted into it at grade A with every read passed and its conviction at the top, and its pool is this many times OBS_MAX_SWAP_USD deep; a hold or a sale is never strong (the signal ledger, read with /signals by the wallets on OBS_CONSOLE_ALLOWLIST) | 20 |
